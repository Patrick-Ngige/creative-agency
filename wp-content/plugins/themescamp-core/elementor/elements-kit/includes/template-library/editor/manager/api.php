...
namespace ElementKit\Includes\TemplateLibrary\Editor\Manager;
...
//        ini_set('display_errors', 1);
        ...
        add_action('wp_ajax_tcg_element_kit_template_library_making_syncing', [$this, 'sync_now']);
...
public function getCategoriesItems() {
    ...
    $typeCondition = is_array($demoDataType) ? "type IN (" . implode(",", $demoDataType) . ")" : "type={$demoDataType}";
...
LEFT JOIN {$catTable} ON {$catTable}.term_id = wp_ep_template_library_cat_post.term_id
 WHERE {$typeCondition} GROUP BY term_id", ARRAY_A);
    ...
    public function get_layouts()
    {
        ...
        if($tab == 'tcg_elementkit_all'){
            ...
            $this->demoType = 4;
        }elseif($tab == 'tcg_elementkit_popup'){
            $this->demoType = 5;
            ...
            $this->demoType = 1;
        }elseif($tab == 'tcg_elementkit_megamenu'){
            $this->demoType = 8;
        }
