...
class ElementKit_Template_Library_Base
{
    ...
    protected $api_url = 'https://fw.themescamp.com/wp-json/templates/v1/';
    ...
    public function createTemplateTables()
    {
        ...
        ;
        if(defined('TCEK_TPL_DB_VER') && TCEK_TPL_DB_VER != get_option('TCEK_TPL_DB_VER',false)){
            ...
            update_option('TCEK_TPL_DB_VER',TCEK_TPL_DB_VER);
        }
    ...
    /**
     * @return array|mixed
     * retrieve element kit categories from remote server with api route
     */
...
$keywordSearch GROUP BY {$postTable}.demo_id $sortingQuery LIMIT $offset, $per_page", ARRAY_A);
    ...
    public function getElementorLibraryData($paged=0){
        ...
        // Adjusting the type condition
        $typeCondition = is_array($demo_tab) ? "type IN (" . implode(",", $demo_tab) . ")" : "type={$demo_tab}";

        if($keywordSearch){
                $keywordSearch .= "AND {$typeCondition} ";
        }else{
                $keywordSearch .= "WHERE {$typeCondition} ";
        }
