...
{ 
	...
	wp_enqueue_style('splitting-cells', get_template_directory_uri() . '/assets/css/splitting-cells.css', array(), '1', 'all' );
        ...
        wp_enqueue_style(TCG_THEME_NAME.'-rtl', get_template_directory_uri() . '/assets/css/theme-rtl.css', array(), '1.0.0', 'all' );
    }
...
function infolio_fonts_url() {
	 ...
	 */
	if ( 'off' !== _x( 'on', 'Google font: on or off', 'infolio' ) ) {
...
*/
function infolio_fonts_style() {
	wp_enqueue_style( 'infolio-fonts', infolio_fonts_url(), array(), '1.0.0' );
}
add_action( 'wp_enqueue_scripts', 'infolio_fonts_style' );

//for google font  in editor
function infolio_fonts_editor_style() {
	$font_url = add_query_arg( 'family', urlencode( 'Open Sans:300,400,400i,500,600,600i,700,700i,800,800i,900|Poppins:300,400,500,600,700,800,900|Barlow Condensed:300,400,500,600,700,800,900' ), "//fonts.googleapis.com/css" );
	add_editor_style( $font_url );
}
add_action( 'after_setup_theme', 'infolio_fonts_editor_style' );

/*
Enqueue admin scripts and styles.
*/
function infolio_admin_style() {
	wp_enqueue_style('infolio-admin-custom', get_stylesheet_directory_uri() . '/assets/css/admin-custom.css', array(), '5.1.1', 'all' );
}
add_action( 'admin_enqueue_scripts', 'infolio_admin_style' );


//Register and load FontAwesome to WP Editor

add_action( 'enqueue_block_editor_assets', 'infolio_editor_fontawesome' );
function infolio_editor_fontawesome() {
